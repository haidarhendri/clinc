...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, username, first_name, last_name')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function get_users_perms($id=null, $dash=false)
	{
		...
		foreach ($groups as $group)
		{
			...
			{
				// get all the things
				$all = $this->db->order_by('id', 'desc')->get($this->tables['permissions'])->result();
	...
	public function build_perms_links($perms)
	{
		...
		foreach($perms as $perm)
		{
			...
			if ($perm->name == 'settings' || $perm->name == 'dashboard')
			{
				...
				{
					$return_arr[] = '<a href="' . site_url('admin/' . $perm->name) . '">' . $perm->description . '</a>';
				}
				// dashboard has a url of /admin
				elseif ($perm->name == 'dashboard')
				{
					$return_arr[] = '<a href="' . site_url('admin') . '">' . $perm->description . '</a>';
				}
			}
			// otherwise, it's all the same
			else
			{
				$return_arr[] = '<a href="' . site_url('admin_' . $perm->name) . '">' . $perm->description . '</a>';
			}
		}
		return $return_arr;
	}
